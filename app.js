...
function drawWrappedTextCentered(ctx, text, x, centerY, maxWidth, lineHeight, maxLines = 2, align = "center") {
  const lines = wrapCanvasText(ctx, text, maxWidth, maxLines);
  ...
  lines.forEach((line, index) => ctx.fillText(line, tx, firstLineY + index * lineHeight));
}
...
function downloadScheduleImage() {
  ...
  days.forEach((day, dayIndex) => {
    ...
    slots.forEach((slot, slotIndex) => {
      ...
      const subjectAreaTop = cardY + 27;
      const subjectAreaBottom = stripY;
      const subjectCenterY = subjectAreaTop + (subjectAreaBottom - subjectAreaTop) / 2;
      ctx.font = "900 17px Arial, sans-serif";
      ctx.textBaseline = "middle";
      drawWrappedTextCentered(ctx, parts.subject, cardX + 10, subjectCenterY, cardW - 20, 20, 2, "center");
      ctx.textBaseline = "top";
